...
public class QuestionsManager {
    ...
    private final List<Review> deletedReviews = new ArrayList<>(); // Stores deleted reviews for restoration
    ...
    public void deleteReview(int reviewId) {
        for (Review r : reviews) {
            if (r.getReviewId() == reviewId) {
                deletedReviews.add(r); // Save to Delete List
                break;
            }
        }
        reviews.removeIf(r -> r.getReviewId() == reviewId);
        System.out.println("Review Deleted (can be restored): ID " + reviewId);
    }

    public void restoreReview(int reviewId) {
        for (Review r : deletedReviews) {
            if (r.getReviewId() == reviewId) {
                reviews.add(r);           // Restore to main list
                deletedReviews.remove(r); // Remove from deleted list
                System.out.println("Review Restored: ID " + reviewId);
                return;
            }
        }
        System.out.println("No deleted review found with ID: " + reviewId);
    }
    ...
    public void banUser(String userName) {
        if (!bannedUsers.contains(userName)) {
            ...
            System.out.println("User " + userName + " is already banned.");
        }
    }
